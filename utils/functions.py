...
def laebls2featuremap_labels(output, labels, stride, sigma=2):
    ...
    batch_i, y, x, cls, w, h = labels[:, 0].astype("int32"), \
                               (labels[:, 3] // stride[-1]).astype("int32"), (labels[:, 2] // stride[0]).astype(
        "int32"), \
    ...
    return t.from_numpy(np.transpose(score_heatmap, (0, 3, 1, 2))).cuda(), t.from_numpy(
        np.transpose(offset_heatmap, (0, 3, 1, 2))).cuda(), \
           t.from_numpy(np.transpose(wh_heatmap, (0, 3, 1, 2))).cuda(), gass_mask.cuda(), t.from_numpy(
        np.transpose(pos_mask, (0, 3, 1, 2))).cuda(), \
...
def draw_box(src, detection, classname, color):
    ...
    for box in detection:
        ...
        cv2.putText(src, classname[cls], (x1, y1 + 20), cv2.FONT_HERSHEY_SIMPLEX, 0.5, color[cls], 2)
...
def nms(detection):
    ...
    while len(detection) > 0:
        ...
        index = (ious < 0.85) & (box[5] == detection[:, 5])
